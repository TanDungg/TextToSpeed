...
export const usePostFriends = (options = {}) => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['Friends', 'list'], exact: false });
      queryClient.invalidateQueries({ queryKey: ['Friends', 'received'], exact: false });
      queryClient.invalidateQueries({ queryKey: ['Friends', 'sent'], exact: false });
    },
    ...options,
  });
};
